...
class DistributedSystemSimulator:
    ...
    def generate_id(self, dc_id, machine_id):
        ...
        if not generator:
            raise ValueError(
                f"No generator found for datacenter {dc_id}, machine {machine_id}"
            )
    ...
    def display_results(self, limit=10):
        ...
        busy_timestamps = sorted(
            timestamp_dist.items(), key=lambda x: x[1], reverse=True
        )[:5]
        ...
        seq_table = []
        for seq, count in sorted(sequence_dist.items())[
            :10
        ]:  # Show first 10 sequence numbers
