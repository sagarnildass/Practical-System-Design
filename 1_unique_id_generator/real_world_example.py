...
class PostService:
    ...
    def list_posts_by_user(self, user_id):
        ...
        with self.lock:
            user_posts = [
                post for post in self.posts.values() if post["user_id"] == user_id
            ]
...
def run_simulation():
    ...
    for user in user_service.list_users():
        ...
        print(
            f"- {user['name']} (ID: {user['id']}, Created: {parsed_id['generated_time']})"
        )
    ...
    for post in all_posts:
        ...
        print(
            f"- {user['name']}: \"{post['content']}\" (ID: {post['id']}, Created: {parsed_id['generated_time']})"
        )
