...
def test_generation_rate(target_rate=10000, duration=1):
    ...
    generators = [SnowflakeIDGenerator(datacenter_id=i % 2, machine_id=i % 16) for i in range(8)]
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=8) as executor:
        while time.time() < end_time:
            ...
            futures = [executor.submit(generate_batch, gen, batch_size) for gen in generators]
    ...
    if rate < target_rate:
        print(f"FAILURE: Generation rate {rate:.2f} IDs/sec is below target {target_rate} IDs/sec")
        return False
    
    print(f"SUCCESS: Generation rate of {rate:.2f} IDs/sec exceeds target {target_rate} IDs/sec")
    return True
