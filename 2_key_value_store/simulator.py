...
class KeyValueStoreSimulator:
    ...
    def setup(self):
        ...
        """
        logger.info("Setting up cluster with %d nodes (replication factor: %d)", 
                   self.num_nodes, self.replication_factor)
    ...
    def demonstrate_conflicts(self):
        ...
        for node in self.nodes:
            ...
            if val is not None:
                logger.info("Node %s has value: %s with context %s", node.node_id, val, ctx)
        ...
        if isinstance(value, list):
            ...
            logger.warning("No conflict detected through standard client interface. Performing manual resolution.")
            resolved_value = max(values)
            logger.info("Manually resolving conflict by taking max value: %s", resolved_value)
            success = self.client.resolve_conflict(conflict_key, resolved_value)
            logger.info("Manual conflict resolution: %s", "Success" if success else "Failed")
    ...
    def demonstrate_node_failure(self):
        ...
        for node in self.nodes:
            logger.info("Node %s membership: %s", 
                       node.node_id, 
        ...
        for i, node in enumerate(self.nodes):
            if i != failed_node_idx and node.is_running:
                ...
                logger.info("Node %s membership: %s", 
                          node.node_id, 
                          list(membership.keys()))
                if failed_node_id not in membership:
                    failure_detected_by.append(node.node_id)
        
        if failure_detected_by:
            logger.info("Failed node %s was properly removed from membership lists by nodes: %s", 
                       failed_node_id, failure_detected_by)
        else:
            logger.warning("Failed node %s was not properly removed from any membership lists", failed_node_id)
