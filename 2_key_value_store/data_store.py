...
class VersionedValue:
    ...
    def __str__(self) -> str:
        ...
        """
        return (
            f"Value: {self.value}, Clock: {self.vector_clock}, Time: {self.timestamp}"
        )
...
class DataStore:
    ...
    def put(
        self, key: str, value: Any, context: Dict[str, int] = None
    ) -> Dict[str, int]:
        """
    ...
    def delete(self, key: str, context: Dict[str, int] = None) -> bool:
        ...
        with self.lock:
            ...
            print(
                f"DataStore({self.node_id}): Deleting key '{key}' with {len(self.data[key])} versions"
            )
    ...
    def _handle_conflicts(self, key: str, new_version: VersionedValue) -> None:
        ...
        for existing_version in self.data[key]:
            comparison = new_version.vector_clock.compare(
                existing_version.vector_clock.clock
            )
