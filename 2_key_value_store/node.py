...
class Node:
    ...
    def __init__(self, node_id: str, coordinator=None):
        ...
        self.membership: Dict[str, Tuple[int, float]] = {}  # {node_id: (heartbeat, timestamp)}
    ...
    def join_cluster(self, coordinator):
        ...
        self.logger.info(f"Node {self.node_id} joined the cluster through coordinator {coordinator.node_id}")
    ...
    def get(self, key: str) -> Tuple[Optional[Any], Optional[Dict[str, int]]]:
        ...
        if results:
            ...
            if len(set(str(v) for v in flattened_values)) > 1:
                latest_context = max([r[1] for r in results if r[1] is not None], 
                                    key=lambda c: sum(c.values()) if c else 0) if results[0][1] else None
        ...
        if self.node_id in responsible_nodes:
            try:
                if self.data_store.delete(key, context):
                    ...
                    self.logger.info(f"Successfully deleted key {key} from local data store")
            except Exception as e:
                self.logger.error(f"Error deleting key {key} from local data store: {e}")
                ...
                if node.node_id == node_id and node.is_running:
                    try:
                        ...
                        if node.data_store.delete(key, context):
                            successful_deletes += 1
                            self.logger.info(f"Successfully deleted key {key} from node {node_id}")
                    except Exception as e:
                        self.logger.error(f"Error deleting key {key} from node {node_id}: {e}")
        
        # For simulation purposes, consider the deletion successful if at least one node deleted it
        if successful_deletes > 0:
            self.logger.info(f"Delete operation completed with {successful_deletes}/{len(responsible_nodes)} successful deletes")
            return True
    ...
    def _gossip_protocol(self):
        ...
        """
        gossip_interval = 0.3  # Send gossip message every 0.3 seconds (increased frequency)
        check_failures_interval = 1.0  # Check for failures every 1 second (more frequent checks)
            ...
            if current_time - last_gossip_time >= gossip_interval:
                ...
                if num_nodes > 0:
                    ...
                    for target_id in target_nodes:
                        ...
                        self.logger.debug(f"Node {self.node_id} sending gossip to {target_id}")
                        # Find the actual node objects and call receive_gossip
                        for node in self.get_all_nodes():
                            if node.node_id == target_id and node.is_running:
                                try:
                                    node.receive_gossip(self.node_id, membership_to_send, failed_nodes_to_send)
                                except Exception as e:
                                    self.logger.error(f"Error sending gossip to {target_id}: {e}")
    ...
    def receive_gossip(self, sender_id: str, remote_membership: Dict[str, Tuple[int, float]], remote_failed_nodes: set = None):
        """
        ...
        with self.membership_lock:
            ...
            if remote_failed_nodes:
                for failed_node in remote_failed_nodes:
                    if failed_node in self.membership and failed_node != self.node_id:
                        self.logger.info(f"Removing node {failed_node} from membership due to failure reported by {sender_id}")
            ...
            if sender_id not in self.membership:
                self.membership[sender_id] = remote_membership.get(sender_id, (0, time.time()))
            ...
            for node_id, (remote_heartbeat, remote_timestamp) in remote_membership.items():
                if node_id == self.node_id:
                    # Don't update our own heartbeat from others
                    continue
                
                # Skip nodes that we know have failed
                if hasattr(self, 'known_failed_nodes') and node_id in self.known_failed_nodes:
                    continue
    ...
    def _check_failed_nodes(self):
        ...
        with self.membership_lock:
            ...
            for node_id, (heartbeat, timestamp) in list(self.membership.items()):
                if node_id != self.node_id and current_time - timestamp > failure_threshold:
                    ...
                    if not node_running:
                        ...
                        self.logger.info(f"Detected failure of node {node_id}, marking for removal. Found: {node_found}, Running: {node_running}")
            ...
            for node_id in failed_nodes:
                if node_id in self.membership:
                    ...
                    self.logger.info(f"Node {node_id} detected as failed and removed from ring")
            ...
            if failed_nodes:
                self.logger.info(f"Current membership list after failure detection: {list(self.membership.keys())}")
